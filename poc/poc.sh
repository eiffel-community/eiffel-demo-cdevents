...
# - Install and configure eiffel-easy2use (https://github.com/eiffel-community/eiffel-easy2use) and
#   eiffel-translator-cdevents (https://github.com/eiffel-community/eiffel-translator-cdevents)
...
while getopts ":c:p:t:d:k:e:o:h" opt; do
  case ${opt} in
    c )
      CLUSTER_NAME=$OPTARG
      ;;
    ...
    k )
      ...
      ;;
    e )
      EIFFEL_TRANSLATOR_HOST=$OPTARG
      ;;
    o )
      EIFFEL_TRANSLATOR_PORT=$OPTARG
      ;;
    h )
      echo "Usage:  poc.sh -e eiffel-translator-host -o eiffel-translator-port"
      echo "Usage:  poc.sh -c cluster-name -p pipeline-version -t triggers-version -d dashboard-version -k knative-version -e eiffel-translator-host -o eiffel-translator-port"	    
      exit 0
      ;;
    \? )
...
if [ -z "$EIFFEL_TRANSLATOR_HOST" ] || [ -z "$EIFFEL_TRANSLATOR_PORT" ]; then
        echo "Missing mandatory arguments for eiffel-translator-cdevents"
        echo "Usage:  poc.sh -e eiffel-translator-host -o eiffel-translator-port"
        exit 1
fi

echo "Eiffel-Translator-CDEvents ==> $EIFFEL_TRANSLATOR_HOST:$EIFFEL_TRANSLATOR_PORT"
...
EIFFEL_TRANSLATOR_URL="http://eiffel-translator-service.default:$EIFFEL_TRANSLATOR_PORT/translate/eiffel"
kubectl create -f - <<EOF || true
apiVersion: eventing.knative.dev/v1
kind: Trigger
metadata:
  name: cd-pipelinerun-finished-to-eiffel
spec:
  broker: events-broker
  filter:
    attributes:
      type: cd.pipelinerun.finished.v1
      pipelinerunname: deploy-spinnaker-poc
  subscriber:
    uri: $EIFFEL_TRANSLATOR_URL
EOF
...
kind: Service
metadata:
   name: eiffel-translator-service
spec:
   type: ClusterIP
   ports:
   - protocol: TCP
     port: $EIFFEL_TRANSLATOR_PORT
     targetPort: $EIFFEL_TRANSLATOR_PORT
EOF
...
## Run the script to configure eiffel-translator with endpoints and triggers
echo "Running script to configure eiffel-translator with endpoints and triggers"
chmod +x $BASE_DIR/eiffel/eiffelConfigPoC.sh $EIFFEL_TRANSLATOR_HOST $EIFFEL_TRANSLATOR_PORT
$BASE_DIR/eiffel/eiffelConfigPoC.sh
